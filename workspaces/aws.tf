...
# Attaches the AWS-managed AdministratorAccess policy to the Terraform Cloud role.
# This grants full permissions across all AWS services and resources.
# Recommended only for testing or development purposes — use scoped policies for production.
resource "aws_iam_role_policy_attachment" "tfc_policy_attachment" {
  role       = aws_iam_role.tfc_role.name
